...
def train_skipgram(model: SkipGramNeg,
    ...
    for epoch in range(epochs):
        for input_words, target_words in tqdm(get_batches(words, batch_size, window_size), desc=f"Epoch: {epoch+1}/{epochs}"):
            steps += 1
            ...
            noise_vectors = model.forward_noise(len(input_words), 5)
            ...
            if steps % print_every == 0:
                ...
                # Cosine similarity
                # TODO
                valid_examples, valid_similarities = cosine_similarity(model.in_embed, 16, 100, device)
