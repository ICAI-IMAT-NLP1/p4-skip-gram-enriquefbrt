...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        out_loss = torch.bmm(output_vectors, input_vectors.transpose(1, 2)).squeeze()
        out_loss = out_loss.sigmoid().log()

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        noise_loss = torch.bmm(noise_vectors.neg(), input_vectors.transpose(1, 2)).squeeze()
        noise_loss = noise_loss.sigmoid().log().sum(1)
